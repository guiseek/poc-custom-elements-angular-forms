...
@Component({
  tag: 'nxc-menu',
  styleUrl: 'nxc-menu.scss',
  shadow: true,
})
export class NxcMenu {
  ...
  getItems() {
    ...
    return [...slot.assignedElements({ flatten: true })].filter(
      (el: any) => el.tagName.toLowerCase() === 'nxc-menu-item' && !el.disabled
    ) as [HTMLNxcMenuItemElement];
  }
  ...
  setActiveItem(item?: HTMLNxcMenuItemElement) {
    this.getItems().map(i => (i.active = i === item));
  }

  scrollItemIntoView(item: HTMLNxcMenuItemElement) {
    if (item) {
  ...
  handleClick(event: MouseEvent) {
    ...
    const item = target.closest('nxc-menu-item');

    if (item && !item.disabled) {
      this.nxcSelect.emit({ item });
    }
  }
